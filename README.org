...
#+begin_src sh
./clime-make.el quickstart --help
#+end_src

#+RESULTS:
#+begin_example shell
Usage: clime-make.el quickstart [OPTIONS] <file>

Scaffold entrypoint + add shebang (scaffold + init)

Arguments:
  <file>    The .el file to set up
...
Options:
  ...
  -L, --load-path VALUE ...        Additional load paths to include in the
                                   shebang
  -R, --rel-load-path VALUE ...    Load path relative (to $(dirname "$0"))
                                   script dir
...
- *:type* --- convert the raw string to a typed value.  Accepts a
  ...
  short aliases ~'int~, ~'num~, ~'str~, ~'bool~), a parameterized
  form (~'(integer :min 1 :max 65535)~), or a composite type
  (~'(member "json" "csv")~, ~'(choice ...)~).  Runs first so that
