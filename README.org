...
Emacs via =clime-invoke=, or (soon) over a network via =clime-serve=.
...
Here's ~clime-make~ (clime's own build tool, built with clime) — same
definition, two surfaces:

[[file:.github/assets/invoke-menu.png]]

#+begin_src sh
./bin/clime-make quickstart --help
#+end_src

#+RESULTS:
#+begin_example shell
Usage: clime-make quickstart [OPTIONS] <file>

Scaffold entrypoint + add shebang (scaffold + init)

Arguments:
  <file>    The .el file to set up (file existing)

Options:
  --self-dir                       Add script's own directory to load path (uses
                                   $(dirname "$0") at runtime)
  -L, --load-path VALUE ...        Additional load paths to include in the
                                   shebang (directory) [$CLIME_LOAD_PATH]
  -R, --rel-load-path VALUE ...    Load path relative to script dir (directory)
                                   [$CLIME_REL_LOAD_PATH=lib/s.el,lib/foo.el]
  --standalone                     Skip the automatic clime load path (for
                                   vendored/bundled setups)
  -f, --force                      Replace an existing non-clime shebang
  -e, --env VALUE ...              Set environment variable in shebang
                                   (NAME=VALUE)
  -o, --output VALUE               Write to OUTPUT instead of modifying the
                                   source file (file)
#+end_example
...
Same definition, different surface.  The handler code doesn't change.
...
** Values map and two-pass pipeline
...
hook between them.  Pass 1 extracts values from argv (type coercion,
static ~:choices~, ~:coerce~) into the values map with source ~user~.
Pass 2 merges env vars, runs ~:conform~ hooks (accumulating errors as
~:error~ entries), applies defaults, and checks required params.  The
~:setup~ hook runs between passes, letting apps load configuration
that influences pass-2 behavior.
...
parsing, interactive menus, and (soon) network servers.  Each surface
