...
- *:type* --- convert the raw string to a typed value.  Accepts a
  symbol (~'integer~, ~'number~, ~'string~) or a function
  ~(string → value)~.  Runs first so that ~:choices~ can compare typed
  values:

  #+begin_src emacs-lisp
  (clime-opt level ("--level") :type 'integer :choices '(1 2 3))

  ;; Function type for custom parsing
  (clime-opt date ("--date") :type #'parse-time-string
    :help "Date in YYYY-MM-DD format")
  #+end_src

  Errors signaled by function types are wrapped as usage errors
  automatically.
