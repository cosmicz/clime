...
*clime* is a declarative CLI framework for Emacs Lisp.  Write your
app's commands, options, and arguments once with a structured DSL
then run it from the shell as an executable script or interactively from
Emacs via =clime-invoke=.

clime handles the plumbing: a polyglot shebang for direct shell
execution, two-pass argument parsing with type coercion, env var
resolution, and validation, auto-generated help, structured output
formats, and a bundler that produces single-file distributions usable
as both standalone CLIs and Emacs libraries.

Here's ~clime-make~ (clime's own build tool, built with clime)
...
Or invoke it interactively from Emacs:
