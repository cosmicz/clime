...
Emacs Lisp is a great language for text processing, system
orchestration, and anything that benefits from deep Emacs integration
--- but building a proper CLI with it means hand-rolling argument
parsing, help text, error messages, and dispatch every time.  *clime*
removes that overhead.  Declare your commands, options, and arguments
in a single ~clime-app~ form and get a complete CLI: parsed and
type-checked arguments, auto-generated ~--help~, environment variable
support, structured output formats, and clean error messages.  Apps
run from the shell via a polyglot shebang or interactively from Emacs
via auto-generated transient menus.
