...
The ~:after-execute~ slot on ~clime-app~ registers hooks that fire
after every handler invocation — CLI, interactive, and HTTP surfaces
all go through ~clime-run--execute~.

Each hook receives three arguments: the context, the integer exit
code, and the wall-clock duration in seconds:
...
*** Unified invocation hooks (audit logging)
