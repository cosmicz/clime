...
literal path segments.  ~_api~ endpoints auto-default to JSON when the
app declares a JSON format.
...
|-----------------------+--------------------+----------------------------------+-----------------------------------------------------------------------|
| ~clime-app~           |                    | Root application definition      | ~:version~, ~:env-prefix~, ~:setup~, ~:config~                      |
