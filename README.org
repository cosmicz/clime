...
(clime-app greet
  ...
  (clime-option shout ("--shout" "-s") :bool
...
#+begin_example
clime-app (root)
  +-- clime-option (--verbose)      app-level option
  +-- clime-command (install)       leaf command
  |     +-- clime-arg (package)
  |     +-- clime-handler
  +-- clime-group (repo)            branch node
        +-- clime-option (--registry)  group-level option
        +-- clime-command (add)
...
category sections in help, epilog text.
...
#+begin_src emacs-lisp
(clime-option token ("--token") :required
...
(clime-mutex output-format
  (clime-option format-json ("--json") :bool)
  (clime-option format-csv  ("--csv")  :bool)
  (clime-option format-text ("--text") :bool))
#+end_src
...
(clime-option trace ("--internal-trace") :bool :hidden
  :help "Enable tracing")
#+end_src
...
#+begin_src emacs-lisp
(clime-option old-flag ("--old") :bool
...
(clime-command copy
  ...
  (clime-option src ("--src") :from verified-id :multiple)
...
| ~clime-option~        | Named option (flag or value)     | ~:bool~, ~:count~, ~:multiple~, ~:separator~, ~:negatable~, ~:required~, ~:requires~, ~:env~, ~:category~ |
| ~clime-mutex~         | Exclusive option group           | Group name, ~:default~, contains ~clime-option~ forms                 |
| ~clime-zip~           | Paired option group              | Group name, contains ~clime-option~ forms                             |
| ~clime-arg~           | Positional argument              | ~:nargs~, ~:required~                                                 |
