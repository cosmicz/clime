...
- *:type* --- convert the raw string to a typed value.  Accepts a
  ...
  a parameterized form (~'(integer :min 1 :max 65535)~), or a
  ...
  ~completing-read~.  Help output shows the resolved type description:

  #+begin_example
  Options:
    --port VALUE      Listen port (integer 1–65535)
    --format VALUE    Output format (json | csv | table)
