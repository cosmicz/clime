...
":"; CLIME_ARGV0="$0" exec emacs --batch -Q \
  ...
  --eval "(with-temp-buffer \
    ...
    (condition-case nil \
      ...
      (end-of-file nil)))" \
  -- "$@" # clime:0.1.1 -*- mode: emacs-lisp; lexical-binding: t; -*-
#+end_src
...
The ~# clime:VERSION~ tag lets ~init~ detect and update existing
shebangs.  ~CLIME_ARGV0~ preserves the original script name for usage
output.
...
| ~clime-option~   | Declare a named option             | ~:flag~, ~:count~, ~:multiple~, ~:type~, ~:choices~, ~:coerce~, ~:env~, ~:category~ |
...
With ~--main FILE~, the bundle includes a guarded entrypoint: the main
file's code is wrapped in ~clime-main-script-p~ and placed inside an
=;;; Entrypoint:= section.  This makes the bundle work as both a library
(~require 'myapp~) and a standalone CLI.

** Entrypoint markers

Source files passed to ~bundle~ may contain an =;;; Entrypoint:= marker
separating library code from entrypoint code:
...
=;;; Entrypoint:=), stripping the entrypoint automatically.  Files
without the marker are extracted up to =;;; ... ends here= as before.

If a source file contains ~(clime-run-batch ...)~ in its library section
(above the =;;; Entrypoint:= marker or without one), ~bundle~ signals an
error --- move it below the marker.

The ~--main~ file must /not/ contain an =;;; Entrypoint:= marker (it is
a plain script whose entire code section becomes the guarded entry).
...
For dual-use bundles, pass ~--env CLIME_MAIN_APP=<feature>~ so the
entry point only activates when run directly (not when loaded as a
library).  The value must match the ~--provide~ feature name.
