...
|--------------------+----------------------------------+-----------------------------------------------------------------------|
| ~clime-app~        | Define a CLI application         | ~:version~, ~:env-prefix~, ~:json-mode~, ~:setup~                    |
| ~clime-command~    | Define a subcommand              | (uses shared command slots)                                           |
| ~clime-group~      | Nest related commands            | ~:inline~                                                             |
| ~clime-option~     | Declare a named option           | ~:bool~, ~:count~, ~:multiple~, ~:separator~, ~:negatable~, ~:mutex~, ~:requires~, ~:env~, ~:category~ |
...
| ~clime-alias-for~  | Alias for a nested command       | Path to target command                                                |
