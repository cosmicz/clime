...
*clime* is a universal declarative command router framework for Emacs Lisp.  Write your
...
them.  Both passes operate on a *values map* --- an alist of
~(NAME . (:value V :source S))~ entries that tracks each parameter's
resolved value and its provenance (~user~, ~app~, ~env~, ~default~,
~conform~).

Pass 1 extracts values from argv (type coercion, static ~:choices~,
~:coerce~) into the values map with source ~user~.  Pass 2 merges
env vars, runs ~:conform~ hooks (accumulating errors as ~:error~
entries), applies defaults, and checks required params.  The ~:setup~
hook runs between passes, letting apps load configuration that
influences pass-2 behavior.

Handlers receive a flat params plist derived from the values map
(via ~clime-values-plist~), so existing handler code is unaffected.
