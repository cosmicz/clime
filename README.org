...
#+begin_src emacs-lisp
(clime-option old-flag ("--old") :flag t
...
#+begin_src emacs-lisp
(clime-option format-json ("--json") :flag t :mutex 'output-format)
(clime-option format-csv  ("--csv")  :flag t :mutex 'output-format)
(clime-option format-text ("--text") :flag t :mutex 'output-format)
#+end_src
...
This implies boolean (no ~:flag t~ needed):
...
When ~--json~ is set, ~clime-output-mode~ is bound to ~'json~ and handler
return values are encoded as JSON.  Handlers can check
~(clime-output-mode-json-p)~ to vary their output format:
...
Get a param value with a default for absent params.  Unlike
~clime-ctx-get~, distinguishes between explicitly ~nil~ and absent:
...
Use ~clime-ctx-get~ or ~clime-let~ for simple binary params, and
~clime-param~ when you need ternary state (common with negatable flags).
...
| ~clime-command~  | Define a subcommand                | ~:help~, ~:aliases~, ~:hidden~, ~:deprecated~, ~:epilog~, ~:category~    |
| ~clime-group~    | Nest related commands              | ~:help~, ~:aliases~, ~:hidden~, ~:deprecated~, ~:inline~, ~:epilog~, ~:category~ |
| ~clime-option~   | Declare a named option             | ~:flag~, ~:count~, ~:multiple~, ~:negatable~, ~:mutex~, ~:type~, ~:choices~, ~:coerce~, ~:conform~, ~:env~, ~:category~, ~:deprecated~, ~:from~ |
| ~clime-arg~      | Declare a positional argument      | ~:nargs~, ~:type~, ~:choices~, ~:coerce~, ~:conform~, ~:default~, ~:help~, ~:deprecated~, ~:from~ |
