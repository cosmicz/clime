...
~:conform~ runs in pass 2 (after env vars and defaults are resolved)
and can both validate and transform the final value.  Unlike ~:coerce~
(which runs immediately during parsing), ~:conform~ sees the fully
resolved value --- including defaults and env var overrides:
...
- Domain validation that depends on the final resolved value
- Normalizing values after all sources (CLI, env, default) are merged
- Cross-cutting checks shared via parameter templates (~:from~)

The pipeline order is: type coercion → ~:coerce~ → choices → ~:conform~.
...
~clime-make.el~ provides two commands: ~init~ (make a script
executable) and ~bundle~ (combine files into one).
