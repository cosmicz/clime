...
~:conform~, ~:mutex~, and defaults.  The ~:setup~ hook runs between
...
exclusion (~:mutex~), option dependencies (~:requires~), paired groups
(~:zip~), value separator (~:separator ","~), choices validation
(static and lazy), custom transforms (~:coerce~, ~:conform~), type
coercion, defaults, hidden options, parameter templates (~:from~).
...
#+begin_src emacs-lisp
(clime-option format-json ("--json") :bool :mutex 'output-format)
(clime-option format-csv  ("--csv")  :bool :mutex 'output-format)
(clime-option format-text ("--text") :bool :mutex 'output-format)
#+end_src
...
in ctx under the group name.  Implies ~:multiple~:
...
| ~clime-option~        | Named option (flag or value)     | ~:bool~, ~:count~, ~:multiple~, ~:separator~, ~:negatable~, ~:required~, ~:requires~, ~:mutex~, ~:zip~, ~:env~, ~:category~ |
